/**
 * Pantalla principal del panel de administracion y auxiliar 
 ...
 * las versiones actuales del ios y android no permite uso de push 
 * 
 */
...
type StatCard = {
    ...
    gradient: [string, string]; //par de colores fopndo principal y fondo secundario
    route: string; //ruta a la que navega al presionar la tarjeta 
    show: boolean; //si el false la tarjeta no se muestra 
};
...
export default function AdminDashboardScreen() {
    ...
    //flags de rol para controlar que se muestra en la pantalla segun rol del usuario
    ...
    /**
     * Estado local
     * objeto con todos los contadores que se muestran en grip de tarjetas
     * valores iniciales en  mientras se cargan 
     */
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                //cada una trae datos de una seccion diferente del panel
                const [cats, subs, prods, orders] = await Promise.all([
                    apiClient.get('/admin/categorias'), 
                    apiClient.get('/admin/subcategorias'),
                    apiClient.get('/admin/productos?limite=1'), // solo se necesita el total
                ...
                // El backend devuelve { data: { totalPedidos, pedidosHoy, ventasTotales, pedidosPorEstado: [...] } }
                const ordData = orders.data?.data || {};
                // Buscar cantidad de pendientes dentro de pedidosPorEstado
                const pedidosPorEstado = Array.isArray(ordData.pedidosPorEstado) ? ordData.pedidosPorEstado : [];
                const pendientesObj = pedidosPorEstado.find((p: any) => p.estado === 'pendiente') || { cantidad: 0 };
                ...
                //los stats quedam en 0 no se muestra mensahe de error al usuario 
                //ignore 
            } finally {
                setLoading(false);
            }
        ...
        /**
         * solo carga las estadisticas si el usuario esta autenticado y tiene rol de admin o auxiliar
         */
  ...
  const cards: StatCard[] = [
    ...
    { title: 'Pendientes',    value: stats.pendientes,    icon: 'time-outline',        gradient: ['#ef4444', '#dc2626'], route: '/admin/pedidos?estado=pendiente',   show: true },
  ];
