...
export default function CheckoutScreen() {
  // Helpers for navigation created inside component to ensure router is ready.
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const backend = e?.responseData;
      if (backend?.errores && Array.isArray(backend.errores) && backend.errores.length > 0) {
        setErrorMessage(backend.errores.join('; '));
      } else if (backend?.message) {
        setErrorMessage(backend.message);
      } else {
        setErrorMessage(e?.message || 'No fue posible confirmar el pedido.');
      }
    } finally {
...
const styles = StyleSheet.create({
  ...
  paymentChipSelected: { borderColor: '#7d2181', backgroundColor: '#f8f2ff' },
