...
const AdminProductosPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('precio', Number.parseFloat(formData.precio));
      formDataToSend.append('stock', Number.parseInt(formData.stock));
  ...
  const handleToggleActivo = async (producto) => {
    try {
      ...
      formDataToSend.append('precio', Number.parseFloat(producto.precio));
      formDataToSend.append('stock', Number.parseInt(producto.stock));
  ...
  const handleActivarDesactivarTodas = async (nuevoEstado) => {
    ...
    try {
      const peticiones = productosAfectados.map(prod => {
        ...
        formDataToSend.append('precio', Number.parseFloat(prod.precio));
        formDataToSend.append('stock', Number.parseInt(prod.stock));
  ...
  const subcategoriasFiltradas = useMemo(() => subcategorias.filter(sub => sub.categoriaId === Number.parseInt(formData.categoriaId)), [subcategorias, formData.categoriaId]);
        ...
        <div className="d-flex gap-2">
          ...
          <Dropdown as={ButtonGroup} className="me-2">
            <Button
              ...
              onClick={async () => { if (tipoExportacion === 'pdf') exportarProductosAPDF(productosFiltradosYOrdenados); else await exportarProductosAExcel(productosFiltradosYOrdenados); }}
            >
            ...
            />
            <Dropdown.Menu>
              <Dropdown.Item as="button" type="button" data-testid="exportar-pdf-option" data-format="pdf" onClick={() => { setTipoExportacion('pdf'); exportarProductosAPDF(productosFiltradosYOrdenados); }}>PDF</Dropdown.Item>
      ...
      <Card className="mb-4 shadow-sm">
        <Card.Body>
          ...
          <Row className="g-3 align-items-end">
            ...
            <Col md={2}><Form.Label className="small">Subcategoría</Form.Label><Form.Select value={filtroSubcategoria} onChange={e => setFiltroSubcategoria(e.target.value)} disabled={!filtroCategoria}><option value="">Todas</option>{filtroCategoria && subcategorias.filter(s => s.categoriaId === Number.parseInt(filtroCategoria) && s.activo).map(sub => <option key={sub.id} value={sub.id}>{sub.nombre}</option>)}</Form.Select></Col>
